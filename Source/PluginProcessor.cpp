...
void LivingLooperAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    while (FifoBuffer_count(hostfifo_p) >= size) {
        ...
        if (model.get() && model->loaded && !mIsMuted.load()) {
            ...
            std::cout << "Living Looper: bypass" << std::endl;
            for (int64_t i=0; i<size*n_loops; ++i) {
                FifoBuffer_write(outputfifo_p, 0.0);
            }
        }
    }   
    // }
    // std::cout << (FifoBuffer_count(outputfifo_p)) << ", " << (n_loops * nSamples) << std::endl;
    if (FifoBuffer_count(outputfifo_p) >= n_loops * nSamples){
        // TODO: currently: mono mix
        for (int i=0; i<nSamples; ++i) {
            auto dry_gain = mSmoothedDryGain.getNextValue();
            auto wet_gain = mSmoothedWetGain.getNextValue();

            float outMix = dry_gain * channelL[i];
            for (int j=0; j<n_loops; ++j){
                float outSample;
                FifoBuffer_read(outputfifo_p, outSample);
                outMix += wet_gain*outSample;
            }
            channelL[i] = outMix;
            channelR[i] = outMix;
        }
